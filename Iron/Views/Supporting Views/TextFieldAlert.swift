...
class TextFieldAlertViewController: UIViewController {
    ...
    private func presentAlertController() {
        ...
        let button = UIAlertAction(title: "Create", style: .default) { [weak self] _ in
            self?.action()
            self?.isPresented?.wrappedValue = false
        }
        let button2 = UIAlertAction(title: "Cancel", style: .default) { [weak self] _ in
            self?.isPresented?.wrappedValue = false
        }
        vc.addAction(button2)
        vc.addAction(button)
